...
class ActionScriptLanguageServerManager extends EventDispatcher implements ILanguageServerManager {
	...
	private static var URI_SCHEMES:Array<String> = [URI_SCHEME_SWC];

	private static var FILE_EXTENSIONS:Array<String> = ['as', 'mxml'];
	...
	private function startNativeProcess():Void {
		...
		if (_model.javaPathForTypeAhead) {
			jdkFolder = try cast(_model.javaPathForTypeAhead.fileBridge.getFile, File) catch (e:Dynamic) null;
		}
	...
	private function javaPathSaveHandler(event:FilePluginEvent):Void {
		...
		if (_model.javaPathForTypeAhead) {
			var javaFile:File = try cast(_model.javaPathForTypeAhead.fileBridge.getFile, File) catch (e:Dynamic) null;
