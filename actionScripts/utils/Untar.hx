...
class Untar {
	...
	private function unTarError(event:Event):Void { //var output:String = process.standardError.readUTFBytes(process.standardError.bytesAvailable);

	}

	private function unTarFileProgress(event:Event):Void { /*var output:String = _process.standardOutput.readUTFBytes(_process.standardOutput.bytesAvailable);
		log(output);*/
