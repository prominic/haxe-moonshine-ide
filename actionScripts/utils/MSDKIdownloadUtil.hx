...
class MSDKIdownloadUtil extends EventDispatcher {
	...
	private var isUpdateChecking:Bool;

	private var isUpdateChecked:Bool;
	...
	private function isNewerVersionFunction(updateVersion:String, currentVersion:String):Bool {
		...
		var tmpSplit2:Array<Dynamic> = currentVersion.split('.');
	...
	private function updateDescriptorLoader_completeHandler(event:Event):Void {
		...
		var updateVersion:String = Std.string(updateDescriptor.node.exe.innerData.node.version.innerData);
		var updateVersionUrl:String = Std.string(updateDescriptor.node.exe.innerData.node.url.innerData);
