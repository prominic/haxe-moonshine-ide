...
class TaskListManager extends EventDispatcher {
	...
	private var killAfterParsingTasks:Bool;
	...
	public function killTasks(value:Array<Dynamic>):Void {
		for (i in Reflect.fields(value)) {
			var tmpItems:Array<Dynamic> = Reflect.field(value, i).split(',');
			var serviceNumber:String = tmpItems[1].substring(1, Reflect.field(tmpItems, i).length - 2);
		...
		// for test/debug, make it 'adl' else 'Moonshine'
		{
			...
			if (Lambda.indexOf(Reflect.field(tmpArr, i), 'adl') != -1) {
				...
				// example
				{
					...
					if (Lambda.indexOf(Reflect.field(tmpArr, i), 'java') != -1) {
						probableJavaServices.push(Reflect.field(tmpArr, i));
					}
				}
