...
import actionScripts.extResources.deng.fzip.utils.ChecksumUtil;
...
class FZipFile {
	...
	private var hasDataDescriptor(get, never):Bool;
	...
	private var _extraFields:haxe.ds.ObjectMap<Dynamic, Dynamic>;
	...
	private var _crc32:Int;

	@:allow(actionScripts.extResources.deng.fzip.fzip)
	private var _sizeCompressed:Int = 0;

	@:allow(actionScripts.extResources.deng.fzip.fzip)
	private var _sizeUncompressed:Int = 0;
	...
	public function new(filenameEncoding:String = 'utf-8') {
		...
		_extraFields = new haxe.ds.ObjectMap<Dynamic, Dynamic>();
	...
	/**
	 * Determine if file is directory based on checksum values
	 */
	private function get_isDirectory():Bool {
		return ((Std.string(_crc32) == '0') && (Std.string(_adler32) == '0'));
	}

	/**
	 * Determine the file's extension value
	 */
	private function get_extension():String {
		if (_filename.indexOf('.') != -1) {
			var tmpSplit:Array<Dynamic> = _filename.split('.');
	...
	public function serialize(stream:IDataOutput, includeAdler32:Bool, centralDir:Bool = false, centralDirOffset:Int = 0):Int {
		...
		var msdosTime:Int = as3hx.Compat.parseInt(as3hx.Compat.parseInt(d.getSeconds()) | (as3hx.Compat.parseInt(d.getMinutes()) << 5)) | as3hx.Compat.parseInt(as3hx.Compat.parseInt(d.getHours()) << 11);
		var msdosDate:Int = as3hx.Compat.parseInt(as3hx.Compat.parseInt(d.getDate()) | (d.getMonth() + 1 << 5)) | as3hx.Compat.parseInt(d.getFullYear() - 1980 << 9);
		...
		// Prep extra fields
		for (headerId in Reflect.fields(_extraFields)) {
			var extraBytes:ByteArray = try cast(_extraFields.get(headerId), ByteArray) catch (e:Dynamic) null;
			if (extraBytes != null) {
				ba.writeShort(as3hx.Compat.parseInt(headerId));
				ba.writeShort(as3hx.Compat.parseInt(extraBytes.length));
		...
		var extrafieldsSize:Int = as3hx.Compat.parseInt(ba.position - filenameSize);
		...
		var commentSize:Int = as3hx.Compat.parseInt(ba.position - filenameSize - extrafieldsSize);
		...
		if (!centralDir && _content.length > 0) {
			if (isCompressed) {
				if (HAS_UNCOMPRESS || HAS_INFLATE) {
					...
					fileSize = as3hx.Compat.parseInt(_content.length - 6);
		...
		var size:Int = as3hx.Compat.parseInt(30 + filenameSize + extrafieldsSize + commentSize + fileSize);
		if (centralDir) {
			size += 16;
		}
		return size;
	...
	private function parseHead(data:IDataInput):Void {
		...
		if (_compressionMethod == COMPRESSION_IMPLODED) {
			...
			_deflateSpeedOption = as3hx.Compat.parseInt(flag & 0x06) >> 1;
		}
		var msdosTime:Int = data.readUnsignedShort();
		var msdosDate:Int = data.readUnsignedShort();
		var sec:Int = as3hx.Compat.parseInt(msdosTime & 0x001f);
		var min:Int = as3hx.Compat.parseInt(msdosTime & 0x07e0) >> 5;
		var hour:Int = as3hx.Compat.parseInt(msdosTime & 0xf800) >> 11;
		var day:Int = as3hx.Compat.parseInt(msdosDate & 0x001f);
		var month:Int = as3hx.Compat.parseInt(msdosDate & 0x01e0) >> 5;
		var year:Int = as3hx.Compat.parseInt(((msdosDate & 0xfe00) >> 9) + 1980);
	...
	private function parseHeadExt(data:IDataInput):Void {
		...
		while (bytesLeft > 4) {
			var headerId:Int = data.readUnsignedShort();
			var dataSize:Int = data.readUnsignedShort();
			...
			if (headerId == 0xdada && dataSize == 4) {
				...
				_extraFields.set(headerId, extraBytes);
			}
			bytesLeft -= as3hx.Compat.parseInt(dataSize + 4);
		}
		if (bytesLeft > 0) {
	 ...
	 */
	@:allow(actionScripts.extResources.deng.fzip.fzip)
	private function parseContent(data:IDataInput):Void {
		if (_compressionMethod == COMPRESSION_DEFLATED && !_encrypted) {
			...
			// Add zlib header
			{
				...
				var flg:Int = as3hx.Compat.parseInt(~_deflateSpeedOption << 6) & 0xc0;
				flg += as3hx.Compat.parseInt(31 - (((0x78 << 8) | flg) % 31));
		...
		if (!isCompressed) {
			if (_content.length > 0) {
				...
				if (HAS_INFLATE) {
					...
					_content.compress();
					_sizeCompressed = as3hx.Compat.parseInt(_content.length - 6);
				}
				_content.position = 0;
				isCompressed = true;
			} else {
