...
class TourDeTextEditor extends BasicTextEditor {
	...
	private var isWebsiteLink:Bool;
	...
	private function onDividerRelease(event:DividerEvent):Void {
		if (event.delta == 0 || Math.isNaN(event.delta)) {
			return;
		}
		...
		cast((swfLoader.content), Object).setActualSize(swfLoader.width, newHeight);
	}

	private function onContentLoaded(event:Event):Void {
		event.target.removeEventListener(Event.COMPLETE, onContentLoaded);
		cast((swfLoader.content), Object).setActualSize(swfLoader.width, swfLoader.height);
	}
