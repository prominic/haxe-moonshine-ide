...
public var files:Array<SVNFileWrapper>;
	...
	private function get_selectedFiles():Array<SVNFileWrapper> {
		...
		for (wrap /* AS3HX WARNING could not determine type for var: wrap exp: EField(EIdent(fileList),dataProvider) type: null */ in fileList.dataProvider) {
			if (!wrap.ignore) {
				files.push(wrap);
			}
		}
	...
	override private function updateDisplayList(unscaledWidth:Float, unscaledHeight:Float):Void {
		...
		if (fileList.dataProvider) {
			var fullHeight:Int = as3hx.Compat.parseInt(16 * fileList.dataProvider.length);
