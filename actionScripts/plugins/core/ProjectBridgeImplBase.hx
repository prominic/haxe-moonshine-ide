...
class ProjectBridgeImplBase {
	...
	private var executeImportArchiveProject:ImportArchiveProject;
	...
	private function deleteFilesAsync():Void {
		if (filesToBeDeleted != null && filesToBeDeleted.length != 0) {
			var tmpWrapper:FileWrapper = try cast(filesToBeDeleted[0], FileWrapper) catch (e:Dynamic) null;
			addRemoveListeners(tmpWrapper.file.fileBridge.getFile, true);

			if (tmpWrapper.file.fileBridge.isDirectory) {
	...
	private function onFileFolderDeletionError(event:Event, showError:Bool = true):Void {
		...
		if (event != null) {
			...
			addRemoveListeners(filesToBeDeleted[0].file.fileBridge.getFile, false);
		}
	}
