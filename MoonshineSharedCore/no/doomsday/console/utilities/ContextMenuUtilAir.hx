...
class ContextMenuUtilAir {
	...
	public static function setUp(console:DConsole, root:DisplayObjectContainer = null):Void {
		...
		if (root.contextMenu == null) {
			root.contextMenu = model.contextMenuCore.getContextMenu();
		}
	...
	private static function getReferences(c:DConsole):Void {
		console = c;
		var a:Array<Dynamic> = console.getManagerRefs();
		scopeManager = a[0];
	...
	private static function onControllerMenu(e:ContextMenuEvent):Void {
		...
		if (Std.is(target, DisplayObject)) {
			...
			var properties:Array<String> = ['name', 'x', 'y', 'width', 'height', 'rotation', 'scaleX', 'scaleY'];
			controllerManager.createController(scopeManager.currentScope.obj, cast properties, e.mouseTarget.x, e.mouseTarget.y);
