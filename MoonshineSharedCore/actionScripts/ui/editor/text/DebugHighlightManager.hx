...
class DebugHighlightManager {
	...
	public static var NONOPENED_DEBUG_FILE_LINE:Int = 0;
	public static var IS_DEBUGGER_CONNECTED:Bool = false;
	...
	public static function verifyNewFileOpen(value:TextEditorModel):Void {
		if (LAST_DEBUG_TEXT_EDITOR_OBJECT != null && (value != LAST_DEBUG_TEXT_EDITOR_OBJECT)) {
			onDebugFinishedEvent(null);
		}
		LAST_DEBUG_TEXT_EDITOR_OBJECT = value;
	}

	private static function onDebugFinishedEvent(event:DebugLineEvent):Void {
		if (LAST_DEBUG_LINE_OBJECT == null) {
			return;
		}
