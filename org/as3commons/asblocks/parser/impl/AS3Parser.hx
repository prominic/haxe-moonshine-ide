...
class AS3Parser extends ParserBase {
	...
	private function parsePackageContent():IParserNode {
		var result:TokenNode = try cast(ASTUtil.newCurlyAST(
				AS3NodeKind.CONTENT, token
		), TokenNode) catch (e:Dynamic) null;
	...
	private function parseMetaData():Node {
		var result:TokenNode = try cast(ASTUtil.newBracketAST(
	...
	private function parseMetaDataParameterList():Node {
		var result:TokenNode = try cast(ASTUtil.newParenAST(
	...
	private function parseOptionalType(node:TokenNode):TokenNode {
		...
		if (tokIs(Operators.COLON)) {
			...
			result = try cast(parseType(), TokenNode) catch (e:Dynamic) null;
		}
	...
	private function parseConditionalExpression():TokenNode {
		var result:TokenNode = try cast(parseOrExpression(), TokenNode) catch (e:Dynamic) null;
	...
	// this would be just a simple name IE 'foo' that means nothing
	 {
		...
		if (tokIs(Operators.LBRACK)) {
			...
			} else if (!Math.isNaN(as3hx.Compat.parseInt(token.text))
				|| !Math.isNaN(as3hx.Compat.parseFloat(token.text))
				|| tokIs(KeyWords.NAN)) {
	...
	private function parseArrayLiteral():TokenNode {
		var result:TokenNode = try cast(ASTUtil.newBracketAST(AS3NodeKind.ARRAY, token), TokenNode) catch (e:Dynamic) null;
	...
	private function parseObjectLiteral():TokenNode {
		var result:TokenNode = try cast(ASTUtil.newCurlyAST(AS3NodeKind.OBJECT, token), TokenNode) catch (e:Dynamic) null;
	...
	private function parseArgumentList():TokenNode {
		var result:TokenNode = try cast(ASTUtil.newParenAST(AS3NodeKind.ARGUMENTS, token), TokenNode) catch (e:Dynamic) null;
	...
	private function parseParameterList():TokenNode {
		var result:TokenNode = try cast(ASTUtil.newParenAST(AS3NodeKind.PARAMETER_LIST, token), TokenNode) catch (e:Dynamic) null;
		consumeParenthetic(Operators.LPAREN); // '('
		consumeWhitespace(result);
		while (!tokIs(Operators.RPAREN)) {
	...
	private function parseDot(node:TokenNode):TokenNode {
		...
		if (tokIs(Operators.LPAREN)) {
			result = try cast(ASTUtil.newParenAST(AS3NodeKind.E4X_FILTER, token), TokenNode) catch (e:Dynamic) null;
			consumeWS(Operators.LPAREN, result);
			result.addChild(node);
			result.addChild(parseExpression());
	...
	private function parseBlock():IParserNode {
		...
		cast((scanner), ISourceCodeScanner).inBlock = true;
		...
		cast((scanner), ISourceCodeScanner).inBlock = false;
	...
	private function parseStatement():IParserNode {
		...
		if (tokIs(KeyWords.FOR)) {
			...
			result = try cast(parseBlock(), TokenNode) catch (e:Dynamic) null;
		} else if (tokIs(KeyWords.VAR)) {
			result = parseVar();
		} else if (tokIs(KeyWords.CONST)) {
			result = parseConst();
			...
			result = try cast(parseExpressionStatement(), TokenNode) catch (e:Dynamic) null;
			if (tokIs(Operators.COLON)) {
	...
	private function parseTraditionalFor(node:TokenNode):TokenNode {
		...
		if (!tokIs(Operators.SEMI)) {
			result = try cast(parseForInit(), TokenNode) catch (e:Dynamic) null;
			if (result != null) {
	...
	private function parseASdoc():TokenNode {
		var sc:ISourceCodeScanner = cast((scanner), ISourceCodeScanner);
		var result:TokenNode = adapter.create(
				AS3NodeKind.AS_DOC,
				token.text,
	...
	public function parseBlockFragment(node:IParserNode):IParserNode {
		...
		cast((scanner), AS3Scanner).moveTo(node.line - 1, node.column);
