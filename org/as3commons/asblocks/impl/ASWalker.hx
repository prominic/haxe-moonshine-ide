...
class ASWalker implements IASWalker {
	...
	public function walkType(element:IType):Void {
		...
		if (Std.is(element, IClassType)) {
			walkClass(cast((element), IClassType));
		} else if (Std.is(element, IInterfaceType)) {
			walkInterface(cast((element), IInterfaceType));
		} else if (Std.is(element, IFunctionType)) {
			walkFunction(cast((element), IFunctionType));
		}
	}
	...
	public function walkClass(element:IClassType):Void {
		...
		var fields:Array<IField> = element.fields;
		len = fields.length;
		for (i in 0...len) {
		...
		var methods:Array<IMethod> = element.methods;
		len = methods.length;
		for (i in 0...len) {
