...
class ASBlocksWalker implements IASBlockVisitor {
	...
	public function visitArrayLiteral(element:IArrayLiteral):Void {
		walkElements(try cast(element.entries, Array/*Vector.<T> call?*/) catch (e:Dynamic) null);
	}

	private function walk(object:Dynamic):Void {
		if (Std.is(object, Vector)) {
			walkElements(try cast(object, Array/*Vector.<T> call?*/) catch (e:Dynamic) null);
		} else {
			walkElement(try cast(object, IScriptNode) catch (e:Dynamic) null);
		}
	}
	...
	private function walkElements(list:Array<IScriptNode>):Void {
		...
		for (i in 0...len) {
			var element:IExpression = try cast(list[i], IExpression) catch (e:Dynamic) null;
			walk(element);
		}
	}
	...
	public function visitTryStatement(element:ITryStatement):Void {
		walkStatementContainer(element);
		var catches:Array<ICatchClause> = element.catchClauses;
		if (catches.length > 0) {
			walk(catches);
		}
