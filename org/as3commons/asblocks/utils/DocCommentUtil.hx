...
class DocCommentUtil {
	...
	public static function newAsDocAST(parent:IParserNode, text:String):IParserNode {
		...
		cast((parent), TokenNode).absolute = true;
		parent.addChildAt(ast, index);
		cast((parent), TokenNode).absolute = false;
	...
	// find the token in the parent
	 {
		...
		if (asdoc == null) {
			...
			cast((comment), DocCommentNode).asdocNode = asdoc;
		} else {
	...
	//var asdoc:IParserNode = buildASDoc(comment.node);
	 {
		...
		var list:TokenNode = try cast(findDoctagList(asdoc), TokenNode) catch (e:Dynamic) null;
		if (list == null) {
			return false;
		}
	...
	public static function hasDocTag(parent:IParserNode, name:String):Bool {
		var list:TokenNode = try cast(findDoctagList(parent), TokenNode) catch (e:Dynamic) null;
		if (list == null) {
			return false;
		}
	...
	private static function _convertDescription(description:String, indent:String):String {
		...
		var split:Array<Dynamic> = description.split('\n');
