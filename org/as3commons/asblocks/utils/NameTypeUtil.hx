...
class NameTypeUtil {
	...
	public static function hasType(ast:IParserNode):Bool {
		var i:ASTIterator = new ASTIterator(ast);
		var result:IParserNode = i.search(AS3NodeKind.TYPE);
		if (result == null) {
			i.reset();
	...
	public static function stringifyType(ast:IParserNode):String {
		...
		while (tok != null && tok.kind != null) {
			if (tok.text != null && tok.text != ':' && tok.kind != 'ws') {
				result += tok.text;
			}
	...
	public static function getQualfiedName(element:IScriptNode):String {
		...
		var type:IType;
		var role:String;
		if (Std.is(element, IField)) {
			var field:IField = cast((element), IField);
			...
			var method:IMethod = cast((element), IMethod);
			role = 'function';
