 ...
 * @productversion 1.0
 */
class MXMLParserImpl implements IMXMLParser {
	...
	public function parse(sourceCode:ISourceCode,
		...
		var classPath:String = null; //entry.filePath.replace(/\\/g, "/");
		var filePath:String = null; //sourceCode.filePath.replace(/\\/g, "/");

		var qualifiedName:String = StringTools.replace(filePath, classPath, '');
		if (qualifiedName.indexOf('/') == 0) {
			qualifiedName = qualifiedName.substring(1, qualifiedName.length);
		}
		qualifiedName = StringTools.replace(qualifiedName, '.mxml', '');
		...
		cast((unit.typeNode), IClassType).superClass = superQualifiedName;
