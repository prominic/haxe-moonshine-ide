	...
	// current token "<!--- doc. -->"
	 {
		...
		asdocLine = cast((scanner), ISourceCodeScanner).commentLine;
		asdocColumn = cast((scanner), ISourceCodeScanner).commentColumn;
			...
			if (tokIs('>')) {
				...
				if (!tokIs('>')) {
					...
					if (token.kind == 'cdata') {
						...
						cast((cast((contentAST), TokenNode).tokenListUpdater), ParentheticListUpdateDelegate).setBoundaries('lcdata', 'rcdata');
