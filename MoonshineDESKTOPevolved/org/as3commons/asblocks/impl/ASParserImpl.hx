 ...
 * @productversion 1.0
 */
class ASParserImpl implements IASParser {
	...
	public function parseAsync(sourceCode:ISourceCode,
			...
			parseBlocks:Bool = true):org.as3commons.asblocks.api.IParserInfo {
		if (entry == null) {
	...
	public function parse(sourceCode:ISourceCode,
			parseBlocks:Bool = true):ICompilationUnit {
		if (parser == null) {
		...
		var ast:IParserNode;
		try {
			ast = parser.buildAst(sourceCode.code.split('\n'), sourceCode.filePath);
	 ...
	 */
	public function parseString(source:String, parseBlocks:Bool = true):ICompilationUnit {
