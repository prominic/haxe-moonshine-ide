 ...
 * @productversion 1.0
 */
class MethodNode extends MemberNode implements IMethod {
	 ...
	 */
	public var hasCode(get, never):Bool;
	private function get_hasCode():Bool {
	 ...
	 */
	public var statements(get, never):Array<IStatement>;
	private function get_statements():Array<IStatement> {
		return cast containerMixin.statements;
	}
	 ...
	 */
	public var accessorRole(get, set):AccessorRole;
	private function get_accessorRole():AccessorRole {
	 ...
	 * @private
	 */
	private function set_accessorRole(value:AccessorRole):AccessorRole {
		if (value.equals(accessorRole)) {
			return value;
		}
		var role:IParserNode = findAccessorRoleNode();
		var ast:IParserNode = ((value == AccessorRole.GETTER)) ? ASTBuilder.newAST(AS3NodeKind.GET, 'get') : ASTBuilder.newAST(AS3NodeKind.SET, 'set');
		if (role.numChildren == 0) {
			role.addChild(ast);
		} else {
	...
	private function get_parameters():Array<IParameter> {
		return cast functionMixin.parameters;
	}
	 ...
	 * @private
	 */
	public function newSuper(arguments:Array<IExpression> = null):ISuperStatement {
		return containerMixin.newSuper(cast arguments);
	}
