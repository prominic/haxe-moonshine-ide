...
class ASTUtil {
	...
	public static function decodeStringLiteral(string:String):String {
		...
		if (string.indexOf('"') != 0 && string.indexOf('\'') != 0) {
			throw new ASBlocksSyntaxError('Invalid delimiter at position 0: ' + Reflect.getProperty(string, Std.string(0)));
		}

		var chars:Array<String> = string.split('');
	...
	public static function escapeString(string:String):String {
		...
		for (i in 0...len) {
			var c:String = Std.string(string.charAt(i));
	...
	private static function escapeEntities(stringToEscape:String):String {
		...
		for (i in 0...stringToEscape.length) {
			var currentCharacter:String = Std.string(stringToEscape.charAt(i));
	...
	public static function constructSyntaxError(statement:String,
		...
		if (Std.is(cause, UnExpectedTokenError)) {
			message = Std.string(cause.message);
	...
	public static function getNodes(kind:String, node:IParserNode):Array<IParserNode> {
		...
		for (i in 0...len) {
			var element:IParserNode = AS3.as(node.children[i], IParserNode);
