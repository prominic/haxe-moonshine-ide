...
class FileUtil {
	...
	public static function readFile(filePath:String):String {
		var fileClass:Class<Dynamic> = as3hx.Compat.castClass(Type.resolveClass('flash.filesystem.File'));
		var fileStreamClass:Class<Dynamic> = as3hx.Compat.castClass(Type.resolveClass('flash.filesystem.FileStream'));
		...
		var data:String = Std.string(stream.readUTFBytes(Reflect.field(stream, 'bytesAvailable')));
	...
	public static function normalizePath(path:String):String {
		if (path.indexOf('\\') != -1) {
			path = new as3hx.Compat.Regex('\\\\', 'g').replace(path, '/');
		}
		return path;
	}
	...
	public static function fileNameFor(unit:ICompilationUnit):String {
		...
		return new as3hx.Compat.Regex('\\.', 'g').replace(name, separator) + '.as';
	}
	...
	public static function createFile(file:File, forceIsDirectory:Bool = false):Void {
		if (!AS3.as(file.parent.exists, Bool)) {
			FileUtil.createFile(file.parent, true);
		}

		if (!AS3.as(file.exists, Bool)) {
			if (AS3.as(file.isDirectory, Bool) || forceIsDirectory) {
