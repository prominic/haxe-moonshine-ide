...
class ModifierUtil {
	...
	public static function hasModifierFlag(node:IParserNode, modifier:Modifier):Bool {
	...
	public static function setModifierFlag(ast:IParserNode, flag:Bool, modifier:Modifier):Void {
	...
	public static function getVisibility(ast:IParserNode):Visibility {
		...
		if (modifiers == null || modifiers.numChildren == 0) {
	...
	private static var modifiers:Array<Dynamic> =
		cast [
		'override',
		'static',
		'final',
	...
	public static function isReservedModifier(modifier:String):Bool {
		for (mod_ in modifiers) {
			var mod:String = cast mod_;
			if (mod == modifier) {
				return true;
			}
		}
	...
	public static function setVisibility(ast:IParserNode, visibility:Visibility):Void {
		...
		if (list == null && !visibility.equals(Visibility.DEFAULT)) {
			var index:Int = (ast.hasKind(AS3NodeKind.META_LIST)) ? 1 : 0;
			if (ast.hasKind(AS3NodeKind.AS_DOC)) {
				index++;
			}
