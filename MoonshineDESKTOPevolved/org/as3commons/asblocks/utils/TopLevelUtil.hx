...
class TopLevelUtil {
	 ...
	 * @private
	 */
	private static var toplevel:Dynamic =
		{
			'ArgumentError': true,
			'arguments': true,
			'Array': true,
			...
			'VerifyError': true,
			'XML': true,
			'XMLList': true// specail // *:true, // void:true, // Null:true
		};
	 ...
	 * toplevel type.
	 */
	public static function isTopLevel(type:String):Bool {
		if (type == '*' || type == 'void' || type == 'Null') {
			return true;
		}

		return Reflect.field(toplevel, type) != null;
	}
