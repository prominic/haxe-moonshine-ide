 ...
 */
class AS3Parser extends ParserBase {
	...
	public static inline var ASDOC_COMMENT:String = '/**';

	public static inline var MULTIPLE_LINES_COMMENT:String = '/*';

	public static inline var SINGLE_LINE_COMMENT:String = '//';

	public static inline var NEW_LINE:String = '\n';

	private static inline var VECTOR:String = 'Vector';
	...
	private var isInFor:Bool = false;
	 ...
	 * @private
	 */
	private var _parseBlocks:Bool = true;
	...
	private function parsePackage():TokenNode {
	 ...
	 */
	@:allow(org.as3commons.asblocks.parser.impl)
	private function parsePackageContent():IParserNode {
		var result:TokenNode = AS3.as(ASTUtil.newCurlyAST(
				AS3NodeKind.CONTENT, token
		), TokenNode);

		var internalParse:Bool =
		tokIs(KeyWords.CLASS) || tokIs(KeyWords.FUNCTION) || tokIs(KeyWords.IMPORT) || tokIs(KeyWords.INCLUDE) || tokIs(KeyWords.USE);
	 ...
	 */
	@:allow(org.as3commons.asblocks.parser.impl)
	private function parseTypeContent():TokenNode {
		var result:TokenNode = AS3.as(ASTUtil.newCurlyAST(
				AS3NodeKind.CONTENT, token
		), TokenNode);
		...
		while (!tokIs(Operators.RCURLY) || configResult != null && tokIs(Operators.RCURLY)) {
			if (tokenStartsWith(ASDOC_COMMENT)) {
				var current:TokenNode = ((pendingMember != null)) ? pendingMember : result;
			...
			} else if (configResult != null && tokIs(Operators.RCURLY)) {
	...
	private function parseMetaData():Node {
		var result:TokenNode = AS3.as(ASTUtil.newBracketAST(
	...
	private function parseMetaDataParameterList():Node {
		var result:TokenNode = AS3.as(ASTUtil.newParenAST(
				AS3NodeKind.PARAMETER_LIST, token
		), TokenNode);
	 ...
	 */
	@:allow(org.as3commons.asblocks.parser.impl)
	private function parseDecList():IParserNode {
		return parseVar();
	}
	...
	private function parseOptionalType(node:TokenNode):TokenNode {
		...
		if (tokIs(Operators.COLON)) {
			...
			result = AS3.as(parseType(), TokenNode);
		}
	 ...
	 */
	@:allow(org.as3commons.asblocks.parser.impl)
	private function parseType():IParserNode {
	 ...
	 */
	@:allow(org.as3commons.asblocks.parser.impl)
	private function parseName():IParserNode {
	...
	private function parseAssignmentExpression():IParserNode {
		...
		while (tokIs(Operators.ASSIGN) || tokIs(Operators.STAR_ASSIGN) || tokIs(Operators.DIV_ASSIGN) || tokIs(Operators.MOD_ASSIGN) || tokIs(Operators.PLUS_ASSIGN) || tokIs(Operators.MINUS_ASSIGN) || tokIs(Operators.SL_ASSIGN) || tokIs(Operators.SR_ASSIGN) || tokIs(Operators.BSR_ASSIGN) || tokIs(Operators.BAND_ASSIGN) || tokIs(Operators.BXOR_ASSIGN) || tokIs(Operators.BOR_ASSIGN) || tokIs(Operators.LAND_ASSIGN) || tokIs(Operators.LOR_ASSIGN)) {
			result.addChild(adapter.copy(Std.string(AS3ParserMap.assignment.getValue(token.text)), token));
			nextTokenConsume(result);
			result.addChild(parseExpression());
		}
	...
	private function parseConditionalExpression():TokenNode {
		var result:TokenNode = AS3.as(parseOrExpression(), TokenNode);
	...
	private function parseEqualityExpression():IParserNode {
		...
		while (tokIs(Operators.EQUAL) || tokIs(Operators.NOT_EQUAL) || tokIs(Operators.STRICT_EQUAL) || tokIs(Operators.STRICT_NOT_EQUAL)) {
			result.addChild(adapter.copy(Std.string(AS3ParserMap.equality.getValue(token.text)), token));
			nextTokenConsume(result);
			result.addChild(parseRelationalExpression());
		}
		return (result.numChildren > 1) ? result : result.getChild(0);
	...
	private function parseRelationalExpression():IParserNode {
		...
		while (!isInFor && tokIs(KeyWords.IN) || tokIs(Operators.LT) || tokIs(Operators.GT) || tokIs(Operators.LE) || tokIs(Operators.GE) || tokIs(KeyWords.IS) || tokIs(KeyWords.AS) || tokIs(KeyWords.INSTANCE_OF)) {
			result.addChild(adapter.copy(Std.string(AS3ParserMap.relation.getValue(token.text)), token));
			nextTokenConsume(result);
			result.addChild(parseShiftExpression());
		}
		return (result.numChildren > 1) ? result : result.getChild(0);
	...
	private function parseShiftExpression():IParserNode {
		...
		while (tokIs(Operators.SL) || tokIs(Operators.SR) || tokIs(Operators.SSL) || tokIs(Operators.BSR)) {
			result.addChild(adapter.copy(Std.string(AS3ParserMap.shift.getValue(token.text)), token));
			nextTokenConsume(result);
			result.addChild(parseAdditiveExpression());
		}
		return (result.numChildren > 1) ? result : result.getChild(0);
	...
	private function parseAdditiveExpression():IParserNode {
		...
		while (tokIs(Operators.PLUS) || tokIs(Operators.MINUS)) {
			result.addChild(adapter.copy(Std.string(AS3ParserMap.additive.getValue(token.text)), token));
			nextTokenConsume(result);
			result.addChild(parseMultiplicativeExpression());
		}
		return (result.numChildren > 1) ? result : result.getChild(0);
	...
	private function parseMultiplicativeExpression():IParserNode {
		...
		|| tokIs(Operators.DIV)
		|| tokIs(Operators.MOD)) {
			result.addChild(adapter.copy(Std.string(AS3ParserMap.multiplicative.getValue(token.text)), token));
			nextTokenConsume(result);
			result.addChild(parseUnaryExpression(result));
		}
		return (result.numChildren > 1) ? result : result.getChild(0);
	 ...
	 */
	@:allow(org.as3commons.asblocks.parser.impl)
	private function parseUnaryExpression(node:TokenNode):IParserNode {
		var result:TokenNode;
	 ...
	 */
	@:allow(org.as3commons.asblocks.parser.impl)
	private function parsePrimaryExpression():TokenNode {
		...
		if (tokIs(Operators.LBRACK)) {
			...
			if (token.text.indexOf(Operators.QUOTE) == 0 || token.text.indexOf(Operators.SQUOTE) == 0) {
				result.kind = AS3NodeKind.STRING;// '' | ""
			} else if (!AS3.as(Math.isNaN(as3hx.Compat.parseInt(token.text)), Bool)
				|| !AS3.as(Math.isNaN(as3hx.Compat.parseFloat(token.text)), Bool)
				|| tokIs(KeyWords.NAN)) {
	...
	private function parseArrayLiteral():TokenNode {
		var result:TokenNode = AS3.as(ASTUtil.newBracketAST(AS3NodeKind.ARRAY, token), TokenNode);
	...
	private function parseObjectLiteral():TokenNode {
		var result:TokenNode = AS3.as(ASTUtil.newCurlyAST(AS3NodeKind.OBJECT, token), TokenNode);
	...
	private function parseArgumentList():TokenNode {
		var result:TokenNode = AS3.as(ASTUtil.newParenAST(AS3NodeKind.ARGUMENTS, token), TokenNode);
	...
	private function parseParameterList():TokenNode {
		var result:TokenNode = AS3.as(ASTUtil.newParenAST(AS3NodeKind.PARAMETER_LIST, token), TokenNode);
		consumeParenthetic(Operators.LPAREN);// '('
		consumeWhitespace(result);
		while (!tokIs(Operators.RPAREN)) {
	...
	private function parseDot(node:TokenNode):TokenNode {
		...
		if (tokIs(Operators.LPAREN)) {
			result = AS3.as(ASTUtil.newParenAST(AS3NodeKind.E4X_FILTER, token), TokenNode);
			consumeWS(Operators.LPAREN, result);
			result.addChild(node);
			result.addChild(parseExpression());
	 ...
	 */
	@:allow(org.as3commons.asblocks.parser.impl)
	private function parseBlock():IParserNode {
		var result:TokenNode = AS3.as(ASTUtil.newCurlyAST(
				AS3NodeKind.BLOCK, token
		), TokenNode);
	 ...
	 */
	@:allow(org.as3commons.asblocks.parser.impl)
	private function parseStatement():IParserNode {
		var result:TokenNode;
		...
		if (tokIs(KeyWords.FOR)) {
			...
			result = AS3.as(parseBlock(), TokenNode);
		} else if (tokIs(KeyWords.VAR)) {
			result = parseVar();
		} else if (tokIs(KeyWords.CONST)) {
			result = parseConst();
			...
			result = AS3.as(parseExpressionStatement(), TokenNode);
			if (tokIs(Operators.COLON)) {
	...
	private function parseTraditionalFor(node:TokenNode):TokenNode {
		...
		if (!tokIs(Operators.SEMI)) {
			result = AS3.as(parseForInit(), TokenNode);
			if (result != null) {
	 ...
	 */
	@:allow(org.as3commons.asblocks.parser.impl)
	private function parseForInit():IParserNode {
	 ...
	 */
	@:allow(org.as3commons.asblocks.parser.impl)
	private function parseForCond():IParserNode {
	 ...
	 */
	@:allow(org.as3commons.asblocks.parser.impl)
	private function parseForIter():IParserNode {
	 ...
	 */
	@:allow(org.as3commons.asblocks.parser.impl)
	private function parseCondition():IParserNode {
		var result:TokenNode = AS3.as(ASTUtil.newParenAST(
				AS3NodeKind.CONDITION, token
		), TokenNode);
	...
	private function parseASdoc():TokenNode {
		...
		sc.commentLine = -1;
		sc.commentColumn = -1;
		result.start = AS3.int(scanner.offset - token.text.length);
	 ...
	 */
	@:allow(org.as3commons.asblocks.parser.impl)
	private function parseExpressionStatement():IParserNode {
	 ...
	 */
	@:allow(org.as3commons.asblocks.parser.impl)
	private function parseMetaDatas():IParserNode {
