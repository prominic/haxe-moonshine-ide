...
class NestedNode {
	...
	public function contains(node:IParserNode):Bool {
		...
		var unique:Array<IParserNode> = cast ASTUtil.getNodes(kind, IParserNode(this));
	...
	public function addChild(child:IParserNode):IParserNode {
		...
		if (_children == null) {
			_children = cast new Array<IParserNode>();
		}

		_children.push(child);

		if (child != null) {
			child.parent = AS3.as(this, IParserNode);
		}

		if (!noUpdate && tokenListUpdater != null) {
			tokenListUpdater.addedChild(AS3.as(this, IParserNode), child);
		}
	...
	public function addChildAt(child:IParserNode, index:Int):IParserNode {
		...
		if (!noUpdate && tokenListUpdater != null) {
			tokenListUpdater.addedChildAt(AS3.as(this, IParserNode), index, child);
		}
	...
	public function removeChild(node:IParserNode):IParserNode {
		...
		for (i in 0...len) {
			if (children[i] == node) {
				...
				if (!noUpdate && tokenListUpdater != null) {
					tokenListUpdater.deletedChild(AS3.as(this, IParserNode), i, node);
					node.parent = null;
				}
				return node;
			}
	...
	public function removeChildAt(index:Int):IParserNode {
		...
		if (!noUpdate && tokenListUpdater != null) {
			tokenListUpdater.deletedChild(AS3.as(this, IParserNode), index, old);
			old.parent = null;
		}
	...
	public function setChildAt(child:IParserNode, index:Int):IParserNode {
		...
		AS3.as(_children[index] = child, IParserNode);
