 ...
 * @productversion 1.0
 */
class MXMLParser extends ParserBase {
	...
	override public function parseCompilationUnit():IParserNode {
		...
		if (tokIs('<')) {
			// first tag is application/component
			{
	...
	* - PROC_INST
	*/
	@:allow(org.as3commons.mxmlblocks.parser.impl)
	private function parseProcInst():TokenNode {
	 ...
	 * @private
	 */
	private var asdocColumn:Int = 0;

	/**
	 * @private
	 */
	private var asdocLine:Int = 0;
	...
	*/
	...
	private function parseASDoc():TokenNode {
		// current token "<!--- doc. -->"
		asdocLine = AS3.int(ISourceCodeScanner(scanner).commentLine);
		asdocColumn = AS3.int(ISourceCodeScanner(scanner).commentColumn);
	...
	*/
	...
	@:allow(org.as3commons.mxmlblocks.parser.impl)
	private function parseTagList():TokenNode {
		// current token "<"
			...
			if (tokIs('>')) {
				...
				if (!tokIs('>')) {
					...
					if (token.kind == 'cdata') {
						...
						ParentheticListUpdateDelegate(TokenNode(contentAST).tokenListUpdater).setBoundaries('lcdata', 'rcdata');
	...
	@:allow(org.as3commons.mxmlblocks.parser.impl)
	private function parseAttList():TokenNode {
	...
	*/
	@:allow(org.as3commons.mxmlblocks.parser.impl)
	private function parseXmlNs():TokenNode {
		// current token "xmlns"
		...
		result.addChild(adapter.create(
						...
						token.line,
						token.column
			));// should +1 since quotes are trimmed;
		result.appendToken(new LinkedListToken(Operators.QUOTE, '"'));
	...
	*/
	@:allow(org.as3commons.mxmlblocks.parser.impl)
	private function parseCData():TokenNode {
	...
	*/
	@:allow(org.as3commons.mxmlblocks.parser.impl)
	private function parseAtt():TokenNode {
	...
	override private function nextTokenConsume(node:TokenNode):Void {
	...
	override private function tokIsWhitespace():Bool {
		return token.text == '\n' || token.text == '\t' ||
