...
class ILanguageServerBridgeImp implements ILanguageServerBridge {
	...
	public function hasCustomTextEditorForUri(uri:String, project:ProjectVO):Bool {
		...
		for (i in 0...managerCount) {
			...
			if (scheme == URI_SCHEME_FILE) {
				var extensionIndex:Int = AS3.int(Lambda.indexOf(manager.fileExtensions, extension));
				if (extensionIndex != -1) {
					return true;
				}
			} else {
				var schemeIndex:Int = AS3.int(Lambda.indexOf(manager.uriSchemes, scheme));
				if (schemeIndex != -1) {
					return true;
				}
			}
	...
	private function removeProjectHandler(event:ProjectEvent):Void {
		var project:ProjectVO = AS3.as(event.project, ProjectVO);
		var managerCount:Int = managers.length;
		for (i in 0...managerCount) {
			var manager:ILanguageServerManager = managers[i];
			if (manager.project == project) {
	...
	private function addProjectHandler(event:ProjectEvent):Void {
		...
		if (project == null || AS3.as(project.projectFolder.projectReference.isTemplate, Bool)) {
			return;
		}
		...
		if (Std.is(project, AS3ProjectVO)) {
			...
			if (AS3.as(as3Project.isVisualEditorProject, Bool)) {
	...
	private function cleanupManager(manager:ILanguageServerManager):Void {
		var index:Int = AS3.int(Lambda.indexOf(managers, manager));
		if (index != -1) {
			return;
		}
		var connectedIndex:Int = AS3.int(Lambda.indexOf(connectedManagers, manager));
		if (connectedIndex != -1) {
			return;
		}
	...
	private function manager_closeHandler(event:Event):Void {
		var manager:ILanguageServerManager = ILanguageServerManager(event.currentTarget);
		var index:Int = AS3.int(Lambda.indexOf(connectedManagers, manager));
		if (index != -1) {
