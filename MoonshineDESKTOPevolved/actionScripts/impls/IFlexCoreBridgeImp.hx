...
class IFlexCoreBridgeImp extends ProjectBridgeImplBase implements IFlexCoreBridge {
	...
	public function testFlashDevelop(file:Dynamic):FileLocation {
		return FlashDevelopImporter.test(AS3.as(file, File));
	}

	public function testFlashBuilder(file:Dynamic):FileLocation {
		return FlashBuilderImporter.test(AS3.as(file, File));
	}

	public function updateFlashPlayerTrustContent(value:FileLocation):Void {
		SWFTrustPolicyModifier.updatePolicyFile(Std.string(value.fileBridge.nativePath));
	}
	...
	public function exportFlashBuilder(project:AS3ProjectVO, file:FileLocation):Void {
		FlashBuilderExporter.export(project, AS3.as(file.fileBridge.getFile, File));
	}
	...
	public function getPluginsNotToShowInSettings():Array<Dynamic> {
		return cast [FileAssociationPlugin, FilesCopyPlugin, ProjectPanelPlugin, ProjectPlugin, HelpPlugin, FindReplacePlugin, FindResourcesPlugin, RecentlyOpenedPlugin, SWFLauncherPlugin, AS3ProjectPlugin, CleanProject, VSCodeDebugProtocolPlugin,
	...
	public function getWindowsMenu():Array<MenuItem> {
		...
		var wmn:Array<MenuItem> = [
				...
				ProjectMenuTypes.JAVA, ProjectMenuTypes.VISUAL_EDITOR_PRIMEFACES, ProjectMenuTypes.VISUAL_EDITOR_FLEX
		], FindReplacePlugin.EVENT_FIND_NEXT,
				'f', [Keyboard.COMMAND],
				'f', [Keyboard.CONTROL]),
				...
				new MenuItem(((AS3.as(ConstantsCoreVO.IS_MACOS, Bool) && !AS3.as(UtilsCore.isSVNPresent(), Bool))) ? 'Grant Permission' : Std.string(resourceManager.getString('resources', 'MANAGE_REPOSITORIES')), null, null, VersionControlEvent.OPEN_MANAGE_REPOSITORIES),
				new MenuItem(null),
				...
				new MenuItem(((AS3.as(ConstantsCoreVO.IS_MACOS, Bool) && !AS3.as(UtilsCore.isGitPresent(), Bool))) ? 'Grant Permission' : Std.string(resourceManager.getString('resources', 'MANAGE_REPOSITORIES')), null, null, VersionControlEvent.OPEN_MANAGE_REPOSITORIES),
				/*new MenuItem(null),
		...
		if (AS3.as(ConstantsCoreVO.IS_AIR, Bool)) {
			...
			projectMenuItems.insert(0, new MenuItem(resourceManager.getString('resources', 'SEARCH_IN_PROJECTS'), null, null, SearchPlugin.SEARCH_IN_PROJECTS,
					'f', [Keyboard.COMMAND, Keyboard.SHIFT],
					'f', [Keyboard.CONTROL, Keyboard.SHIFT]));
		}
		...
		/* AS3HX WARNING namespace modifier CONFIG::OSX */{
			...
			for (i in ...firstMenuItems.length) {
				if (firstMenuItems[i].label == 'Close All') {
					firstMenuItems.insert(i + 1, new MenuItem(null));
					firstMenuItems.insert(i + 2, new MenuItem('Access Manager', null, null, ProjectEvent.ACCESS_MANAGER));
					firstMenuItems.insert(i + 3, new MenuItem((AS3.as(ConstantsCoreVO.IS_BUNDLED_SDK_PRESENT, Bool)) ? 'Extract Bundled SDK' : 'Moonshine Helper Application', null, null, (AS3.as(ConstantsCoreVO.IS_BUNDLED_SDK_PRESENT, Bool)) ? StartupHelperEvent.EVENT_SDK_UNZIP_REQUEST : StartupHelperEvent.EVENT_MOONSHINE_HELPER_DOWNLOAD_REQUEST));
					break;
				}
			}
		}
	...
	public function reAdjustApplicationSize(width:Float, height:Float):Void {
		var tmpStage:Stage = AS3.as(FlexGlobals.topLevelApplication.stage, Stage);
	...
	public function untar(fileToUnzip:FileLocation, unzipTo:FileLocation, unzipCompleteFunction:Function, unzipErrorFunction:Function = null):Void {
		var tmpUnzip:Untar = new Untar(fileToUnzip, unzipTo, cast unzipCompleteFunction, cast unzipErrorFunction);
	}
	...
	private function get_runtimeVersion():String {
		return Std.string(NativeApplication.nativeApplication.runtimeVersion);
	}

	public var version(get, never):String;
	@:access(FastXML) private function get_version():String {
		var appDescriptor:FastXML = NativeApplication.nativeApplication.applicationDescriptor;
		var ns:Namespace = new Namespace(appDescriptor.node.namespace());
		var appVersion:String = Std.string(appDescriptor.versionNumber);
