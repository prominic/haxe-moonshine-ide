...
class CreateCertificate extends PluginBase {
	...
	public static var ASCRIPTLINES(default, never):FastXML = FastXML.parse('<root><![CDATA[
							#!/bin/bash
							on run argv
	...
	public function buildCertificate():Void {
		if (nativeProcess == null) {
			if (!AS3.as(IDEModel.getInstance().defaultSDK, Bool)) {
	...
	private function shellData(e:ProgressEvent):Void {
		...
		match = as3hx.Compat.match(data, new as3hx.Compat.Regex('nativeProcess: Target \\d not found', ''));
		if (match != null) {
