...
class TourDeTreeItemRenderer extends TreeItemRenderer {
	...
	override private function updateDisplayList(unscaledWidth:Float, unscaledHeight:Float):Void {
		...
		if (Std.is(model.activeEditor, TourDeTextEditor) && AS3.as(TourDeTextEditor(model.activeEditor).currentFile, Bool)) {
			var pattern:as3hx.Compat.Regex = new as3hx.Compat.Regex(new as3hx.Compat.Regex('(\\\\)', 'g'));
			var nodeApp:String = Std.string(data.att.app);
			var pathNode:String = Std.string(TourDeTextEditor(model.activeEditor).currentFile.fileBridge.nativePath.replace(pattern, '/'));
