...
class JavaLanguageServerManager extends EventDispatcher implements ILanguageServerManager {
	...
	private static var URI_SCHEMES(default, never):Array<String> = [];
	private static var FILE_EXTENSIONS(default, never):Array<String> = ['java'];
	...
	private function prepareApplicationStorage():Void {
		...
		try {
			if (AS3.as(storageFolder.exists, Bool)) {
		...
		if (showStorageError || !AS3.as(storageFolder.exists, Bool) || !AS3.as(jarFile.exists, Bool)) {
	...
	private function startNativeProcess():Void {
		...
		var jdkPath:String = Std.string(GetProjectSDKPath.getProjectSDKPath(_project, _model));
	...
	private function getWorkspaceNativePath():String {
		...
		var projectPath:String = Std.string(_project.folderLocation.fileBridge.nativePath);
	...
	private function jdkPathSaveHandler(event:FilePluginEvent):Void {
		...
		if (GetProjectSDKPath.getProjectSDKPath(_project, _model) != _previousJDKPath) {
	...
	private function executeLanguageServerCommandHandler(event:ExecuteLanguageServerCommandEvent):Void {
		...
		switch (event.command) {
			...
			case COMMAND_JAVA_APPLY_WORKSPACE_EDIT:
				event.preventDefault();
				var workspaceEdit:WorkspaceEdit = WorkspaceEdit.parse(Reflect.getProperty(event.arguments, Std.string(0)));
	...
	private function language__status(message:Dynamic):Void {
		...
		switch (Reflect.field(Reflect.field(message, 'params'), 'type')) {
			case 'Starting':
				GlobalEventDispatcher.getInstance().dispatchEvent(new StatusBarEvent(
						StatusBarEvent.LANGUAGE_SERVER_STATUS,
						'Java', Reflect.field(Reflect.field(message, 'params'), 'message'), false));
			case 'Message':
				GlobalEventDispatcher.getInstance().dispatchEvent(new StatusBarEvent(
						StatusBarEvent.LANGUAGE_SERVER_STATUS,
						'Java', Reflect.field(Reflect.field(message, 'params'), 'message'), false));
			...
			case _:
				{
					trace('Unknown ' + METHOD_LANGUAGE__STATUS + ' message type:', Reflect.field(Reflect.field(message, 'params'), 'type'));
					break;
				}
		}
	}
	...
	private function language__actionableNotification(notification:Dynamic):Void {
		var params:Dynamic = Reflect.field(notification, 'params');
		var severity:Int = AS3.int(Reflect.field(notification, 'severity'));
		var message:String = AS3.string(Reflect.field(params, 'message'));
		var commands:Array<Dynamic> = cast AS3.asArray(Reflect.field(params, 'commands'));
		...
		for (i in 0...commandCount) {
			...
			var title:String = Std.string(Reflect.field(command, 'title'));
			var commandName:String = AS3.string(Reflect.field(command, 'command'));
			var args:Array<Dynamic> = cast AS3.asArray(Reflect.field(command, 'arguments'));
		...
		PopUpManager.addPopUp(popup, AS3.as(FlexGlobals.topLevelApplication, DisplayObject), true);
		popup.y = ((AS3.as(ConstantsCoreVO.IS_MACOS, Bool))) ? 25 : 45;
